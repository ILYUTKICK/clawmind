...
export function AgentReasoningFlow({ steps, isLoading }: AgentReasoningFlowProps) {
  ...
  const agentOrder: AgentStep["name"][] = [
    "memory_retrieval", "planner", "researcher", "risk_agent",
    "architect", "critic", "final_agent", "memory_writer",
  ];

  const completedCount = agentOrder.filter((name) => completedAgents.has(name)).length;
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-zinc-100">Agent Reasoning Flow</h2>
          <p className="mt-1 text-sm text-zinc-400">
            Multi-agent orchestration with data flow visualization.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {completedCount > 0 && (
            <span className="text-xs font-mono text-zinc-500">{completedCount}/{totalCount}</span>
          )}
          {isLoading && (
      ...
      <div className="relative">
        {agentOrder.map((agentName, index) => {
          ...
          const label = step?.label ?? agentName.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
          const incomingFlows = DATA_FLOWS.filter((f) => f.to === agentName);
          const outgoingFlows = DATA_FLOWS.filter((f) => f.from === agentName);
                    ...
                    return (
                      <span
                        ...
                        className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-mono ${
                          fromCompleted ? `${flow.color} border-white/10 bg-white/[0.03]` : "text-zinc-600 border-white/5 bg-transparent"
                        }`}
                      >
                        <svg width="14" height="10" viewBox="0 0 14 10" className={fromCompleted ? "opacity-80" : "opacity-20"}>
                          <line x1="0" y1="5" x2="9" y2="5" stroke={strokeColor} strokeWidth="1.5" strokeDasharray={fromCompleted ? "none" : "2 2"} />
                          <polygon points="9,2 14,5 9,8" fill={strokeColor} />
                        </svg>
                        {flow.label}
                      </span>
                    );
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="relative flex-shrink-0">
                      ...
                      {isRunning && <span className="absolute inset-0 animate-ping rounded-full bg-cyan-400/20" />}
                    </div>
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold truncate ${isCompleted ? "text-white" : isRunning ? "text-cyan-200" : "text-zinc-400"}`}>
                        {index + 1}. {label}
                      </p>
                      <p className="text-[10px] font-mono text-zinc-500 truncate">{skill}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold ${statusBadgeClass(status)}`}>
                    ...
                    <svg className={`h-3.5 w-3.5 text-zinc-500 transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {isExpanded && step?.output && (
                  <div className="mt-3 ml-8 rounded-xl border border-white/10 bg-black/30 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Raw Output</span>
                      {step.startedAt && <span className="text-[10px] font-mono text-zinc-600">{step.startedAt}</span>}
                    </div>
                    <pre className="max-h-64 overflow-auto whitespace-pre-wrap text-xs leading-5 text-zinc-400 scrollbar-thin">{step.output}</pre>
                  </div>
                )}
              ...
              {index < agentOrder.length - 1 && (
                <div className="flex items-center justify-center py-0.5">
                  <div className="flex flex-col items-center">
                    <div className={`w-px h-5 transition-colors duration-500 ${completedAgents.has(agentName) ? "bg-emerald-400/30" : isRunning ? "bg-cyan-400/20" : "bg-white/5"}`} />
                    {completedAgents.has(agentName) && <div className="h-1 w-1 rounded-full bg-emerald-400/40" />}
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-white/5 pt-3">
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500"><span className="text-emerald-400">✓</span> Completed</div>
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500"><span className="text-cyan-400 animate-pulse">●</span> Running</div>
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500"><span className="text-zinc-500">○</span> Pending</div>
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
          <svg width="14" height="10" viewBox="0 0 14 10"><line x1="0" y1="5" x2="9" y2="5" stroke="#a1a1aa" strokeWidth="1.5" /><polygon points="9,2 14,5 9,8" fill="#a1a1aa" /></svg>
          Data flow
        </div>
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500"><span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-400/40" /> Active connection</div>
      </div>
    </section>
  );
}