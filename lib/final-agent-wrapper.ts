...
export async function runFinalAgent(input: FinalAgentInput): Promise<{
  ...
  const modelLabel = input.model ?? "deepseek/deepseek-chat-v3-0324";
  ...
  // ── Structured Output Validation with Repair Retry ──
  // If the model returned invalid JSON (fallback was used), retry once with repair prompt
  if (report.evidence[0] === "Report generation mode: FALLBACK_RISK_BASED_SCORE.") {
    console.warn(`[Final Agent] JSON parse failed — attempting repair retry with ${modelLabel}`);

    try {
      const repairOutput = await runInference({
        agentName: "final_agent",
        systemPrompt: [
          "You are a JSON repair agent. The previous model output was invalid JSON.",
        ...
        userPrompt: [
          ...
          "",
          "Return a valid JSON object based on the above content. Fix any JSON syntax errors.",
        ].join("\n"),
      ...
      if (repairedReport.evidence[0] === "Report generation mode: MODEL_JSON.") {
        console.log("[Final Agent] Repair retry succeeded — valid JSON obtained");
        return {
          rawOutput: repairOutput,
          report: {
            ...repairedReport,
            evidence: [
              "Report generation mode: MODEL_JSON_REPAIR_RETRY.",
              ...repairedReport.evidence.slice(1),
            ],
          },
        };
      }

      console.warn("[Final Agent] Repair retry also failed — using fallback report");
    } catch (repairErr) {
      console.warn("[Final Agent] Repair retry exception:", repairErr);
    }
  }
